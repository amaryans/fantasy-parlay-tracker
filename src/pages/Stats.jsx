...
import { formatAmerican, formatMoney, formatPct, parlayOdds } from '../lib/odds.js'
import { computeLeagueStats, computeMemberStats, rankMembers } from '../lib/stats.js'
import { weekLabel } from '../lib/week.js'
import { ResultBadge } from '../components/Badges.jsx'

export default function Stats() {
  const { profiles, weeks, legs, legsForWeek, nameOf, me } = useLeague()
    ...
    <div className="stack">
      <div className="card">
        ...
        <div className="stat-tiles">
          <div className="tile">
            ...
            <div className="sub">{league.won} won · {league.lost} lost</div>
          </div>
          <div className="tile">
          ...
          <div className="tile">
            ...
            <div className={`value ${league.net > 0 ? 'success' : league.net < 0 ? 'error' : ''}`}>{formatMoney(league.net)}</div>
          ...
          {league.biggestHit && (
            <div className="tile">
              ...
              <div className="sub">{weekLabel(league.biggestHit.week.week)} · {nameOf(league.biggestHit.week.loser_id)}</div>
            </div>
          )}
        </div>
        {league.placed > 0 && (
          <div className="grid mt">
            {mostLoser.top > 0 && <Superlative label="Most parlays placed" name={mostLoser.names} detail={`${mostLoser.top} time${mostLoser.top === 1 ? '' : 's'} as the loser`} />}
            {luckiest && <Superlative label="Best luck as placer" name={luckiest.names} detail={`${formatMoney(luckiest.top)} net`} />}
            {sinker.top > 0 && <Superlative label="Parlay killer" name={sinker.names} detail={`${sinker.top} losing leg${sinker.top === 1 ? '' : 's'}`} />}
          </div>
        )}
      </div>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {members.map((m, i) => (
                <tr key={m.profile.id} className={m.profile.id === me.id ? 'me' : m.won + m.lost === 0 ? 'dim' : ''}>
                  <td className="rank">{m.won + m.lost >= 3 ? i + 1 : '–'}</td>
                  <td className="nowrap">{m.profile.display_name}</td>
                  <td className="num nowrap">{m.won}-{m.lost}-{m.push}{m.pending ? <span className="muted"> ({m.pending} open)</span> : null}</td>
                  <td className="num">{formatPct(m.hitRate)}</td>
                  <td className="num">{formatAmerican(m.avgOdds)}</td>
                  <td className={`num ${m.edge > 0 ? 'success' : m.edge < 0 ? 'error' : ''}`}>{m.edge === null ? '—' : `${m.edge > 0 ? '+' : ''}${(m.edge * 100).toFixed(0)}`}</td>
                  <td className={`num streak ${m.currentStreak >= 3 ? 'hot' : m.currentStreak <= -3 ? 'cold' : ''}`}>
                    {m.currentStreak === 0 ? '—' : m.currentStreak > 0 ? `W${m.currentStreak}` : `L${-m.currentStreak}`}
                  </td>
                  <td className="num">{m.bestStreak ? `W${m.bestStreak}` : '—'}</td>
                  <td className="num">{m.timesLoser}</td>
                  <td className={`num ${m.net > 0 ? 'success' : m.net < 0 ? 'error' : ''}`}>{m.timesLoser ? formatMoney(m.net) : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <p className="muted small mt">
          "vs implied" compares each member's actual hit rate with the win probability their average odds implied.
          Positive means they beat the book. "Placer net" is money won minus staked on the weeks they were the loser.
        </p>
      </div>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>Week</th><th>Placed by</th><th className="num">Legs</th><th className="num">Odds</th><th className="num">Stake</th><th className="num">Payout</th><th>Result</th></tr>
            </thead>
            <tbody>
              {settledWeeks.length === 0 && <tr><td colSpan={7} className="muted">No settled parlays yet.</td></tr>}
              {settledWeeks.map((w) => {
                const wl = legsForWeek(w.id)
                const { american, decimal } = parlayOdds(wl)
                const payout = w.parlay_result === 'won' ? (w.payout ?? (decimal ? decimal * Number(w.stake) : null)) : null
                return (
                  <tr key={w.id}>
...
function Superlative({ label, name, detail }) {
  return (
    <div className="tile">
