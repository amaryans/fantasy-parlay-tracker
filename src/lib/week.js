...
export function weekStartDate(seasonStart, week) {
  const tuesday = parseLocalDate(seasonStart)
  tuesday.setDate(tuesday.getDate() - 2 + (week - 1) * 7)
  return tuesday
}

export function nflWeekFor(date, seasonStart) {
...
export function weekLabel(week) {
  ...
  const names = { 19: 'Wild Card', 20: 'Divisional', 21: 'Conf. Champ', 22: 'Super Bowl' }
  return names[week] || `Week ${week}`
}

export function isLocked(week) {
  return Boolean(week?.lock_at) && new Date(week.lock_at).getTime() <= Date.now()
}
