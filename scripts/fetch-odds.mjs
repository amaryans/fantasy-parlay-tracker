...
// Same week math as src/lib/week.js: weeks run Tuesday -> Monday, and
...
async function main() {
  const { data: settings, error: settingsError } = await supabase
    .from('league_settings').select('season, season_start').eq('id', 1).single()
  ...
  console.log(`Fetched ${events.length} events. Requests remaining: ${res.headers.get('x-requests-remaining')}`)
  ...
  console.log(`Upserted ${gamesUpserted} games and ${oddsUpserted} odds rows for weeks ${currentWeek}-${currentWeek + 1}.`)
}
